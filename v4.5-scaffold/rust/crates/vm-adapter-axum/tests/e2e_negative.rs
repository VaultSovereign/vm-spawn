...
#[tokio::test]
async fn tampered_content_digest_yields_unauthorized() {
    ...
    builder.push_labeled("job_id", b"neg");
    builder.push_labeled("final_value", b"13");
    ...
    let nonce = "nonce-negative";
    sign_request_with(&mut http_req, &body_bytes, &kp, created, Some(nonce))
        .expect("sign request");

    // Tamper with the content-digest header after signing.
    let mut tampered_headers = http_req.headers().clone();
    tampered_headers.insert(
        "content-digest",
        "sha-256=:ZmFrZURpZ2VzdA==:".parse().expect("header value"),
    );

    let mut reqwest_headers = ReqwestHeaderMap::new();
    for (name, value) in tampered_headers.iter() {
